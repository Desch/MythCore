...
bool PathFinder::calculate(float destX, float destY, float destZ, bool forceDest)
{
    ...
    sLog->outDebug(LOG_FILTER_PATHFINDING, "++ PathFinder::calculate() for %u \n", m_sourceUnit->GetGUIDLow());
    ...
    if(inRange(oldDest, dest, dist, dist) && m_pathPoints.size() > 2)
    {
        ...
        sLog->outDebug(LOG_FILTER_PATHFINDING, "++ PathFinder::calculate:: precalculated path\n");
...
void PathFinder::createFilter()
{
    ...
    if(m_sourceUnit->GetTypeId() == TYPEID_UNIT)
    {
        const Creature* creature = m_sourceUnit->ToCreature();
        if(creature->canWalk())
