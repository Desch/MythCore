...
{
    float threat = 0.0f;
    HostileReference* pHostileReference = iThreatContainer.getReferenceByTarget(victim);
    if(!pHostileReference && alsoSearchOfflineList)
        pHostileReference = iThreatOfflineContainer.getReferenceByTarget(victim);
    if(pHostileReference)
        threat = pHostileReference->getThreat();
    return threat;
}
...
{
    if(HostileReference* pHostileReference = iThreatContainer.getReferenceByTarget(pTaunter))
        pHostileReference->resetTempThreat();
}
