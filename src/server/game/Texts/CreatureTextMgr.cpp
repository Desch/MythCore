    ...
    switch(range)
    {
        ...
        case TEXT_RANGE_WORLD:
        {
            ...
            for(SessionMap::const_iterator iter = smap.begin(); iter != smap.end(); ++iter)
            {
                ...
                {
                    if(data->GetOpcode() == SMSG_MESSAGECHAT)//override whisperguid with actual player's guid
                        data->put<uint64>(1+4+8+4+4+(int32)(strlen(source->GetName())+1), uint64(pPlayer->GetGUID()));
                    if(pPlayer->GetSession()  && (!team || Team(pPlayer->GetTeam()) == team) && (!gmOnly || pPlayer->isGameMaster()))
