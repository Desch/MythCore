...
bool ReputationMgr::SetReputation(FactionEntry const* pFactionEntry, int32 standing, bool incremental)
{
    if(!pFactionEntry)
        return false;

    sScriptMgr->OnPlayerReputationChange(m_player, pFactionEntry->ID, standing, incremental);
    bool res = false;
    // if spillover definition exists in DB, override DBC
    if(const RepSpilloverTemplate *repTemplate = sObjectMgr->GetRepSpilloverTemplate(pFactionEntry->ID))
    {
    ...
    else
    {
        ...
        SimpleFactionsList const* flist = GetFactionTeamList(pFactionEntry->ID);
        // if has no sub-factions, check for factions with same parent
        if(!flist && pFactionEntry->team && pFactionEntry->spilloverRateOut != 0.0f)
        {
            spillOverRepOut *= pFactionEntry->spilloverRateOut;
            if(FactionEntry const *parent = sFactionStore.LookupEntry(pFactionEntry->team))
            {
                ...
                if(parentState != m_factions.end() && (parentState->second.Flags & FACTION_FLAG_SPECIAL))
                    ...
                    flist = GetFactionTeamList(pFactionEntry->team);
            }
        }
        if(flist)
        {
            ...
            for(SimpleFactionsList::const_iterator itr = flist->begin(); itr != flist->end(); ++itr)
            {
                ...
                {
                    if(factionEntryCalc == pFactionEntry || GetRank(factionEntryCalc) > ReputationRank(factionEntryCalc->spilloverMaxRankIn))
                        continue;
    ...
    FactionStateList::iterator faction = m_factions.find(pFactionEntry->reputationListID);
    if(faction != m_factions.end())
    {
        res = SetOneFactionReputation(pFactionEntry, standing, incremental);
