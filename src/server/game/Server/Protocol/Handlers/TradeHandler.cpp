...
static void setAcceptTradeMode(TradeData* myTrade, TradeData* hisTrade, Item **myItems, Item **hisItems)
{
    ...
    for(uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        {
            sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_PLAYER_ITEMS), "player trade item %u bag: %u slot: %u", item->GetGUIDLow(), item->GetBagSlot(), item->GetSlot());
            //Can return NULL
            myItems[i] = item;
        ...
        {
            sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_PLAYER_ITEMS), "partner trade item %u bag: %u slot: %u", item->GetGUIDLow(), item->GetBagSlot(), item->GetSlot());
            hisItems[i] = item;
