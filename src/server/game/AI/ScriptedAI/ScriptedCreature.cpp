...
void SummonList::DespawnEntry(uint32 entry)
{
    for(iterator i = begin(); i != end();)
    {
        Creature* pSummoned = Unit::GetCreature(*me, *i);
        if(!pSummoned)
            erase(i++);
        else if(pSummoned->GetEntry() == entry)
        {
            erase(i++);
...
void SummonList::DespawnAll()
{
    while(!empty())
    {
        Creature* pSummoned = Unit::GetCreature(*me, *begin());
        if(!pSummoned)
            erase(begin());
        else
        {
...
void ScriptedAI::DoTeleportTo(float fX, float fY, float fZ, uint32 uiTime)
{
    ...
    else
    {
        ...
        me->MonsterMoveWithSpeed(fX, fY, fZ, speed);
    }
}
...
void ScriptedAI::DoTeleportAll(float fX, float fY, float fZ, float fO)
{
    ...
    for(Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
        if(Player* pPlayer = i->getSource())
            if(pPlayer->isAlive())
                pPlayer->TeleportTo(me->GetMapId(), fX, fY, fZ, fO, TELE_TO_NOT_LEAVE_COMBAT);
}
...
BossAI::BossAI(Creature* pCreature, uint32 bossId) : ScriptedAI(pCreature),
...
WorldBossAI::WorldBossAI(Creature* pCreature) : ScriptedAI(pCreature), summons(pCreature) { }
