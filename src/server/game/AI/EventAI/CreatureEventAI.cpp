...
bool CreatureEventAI::CanCast(Unit* Target, SpellEntry const *Spell, bool Triggered)
{
    ...
    SpellRangeEntry const* pSpellRange = sSpellRangeStore.LookupEntry(Spell->rangeIndex);
    //Spell has invalid range store so we can't use it
    if(!pSpellRange)
        return false;

    //Unit is out of range of this spell
    if(!me->IsInRange(Target, pSpellRange->minRangeHostile, pSpellRange->maxRangeHostile))
        return false;
