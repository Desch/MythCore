    ...
    do
    {
        ...
        switch(temp.event_type)
        {
            ...
            case EVENT_T_SPELLHIT:
                ...
                {
                    SpellEntry const* pSpellEntry = sSpellStore.LookupEntry(temp.spell_hit.spellId);
                    if(!pSpellEntry)
                    {
                    ...
                    if((temp.spell_hit.schoolMask & pSpellEntry->SchoolMask) != pSpellEntry->SchoolMask)
            ...
            {
                SpellEntry const* pSpellEntry = sSpellStore.LookupEntry(temp.buffed.spellId);
                if(!pSpellEntry)
                {
                    sLog->outErrorDb("CreatureEventAI:  Creature %u has non-existant SpellID(%u) defined in event %u.", temp.creature_id, temp.spell_hit.spellId, i);
                    continue;
        ...
        for(uint32 j = 0; j < MAX_ACTIONS; j++)
        {
            ...
            switch(action.type)
            {
                ...
                case ACTION_T_CAST:
                {
                    const SpellEntry* pSpellEntry = sSpellStore.LookupEntry(action.cast.spellId);
                    if(!pSpellEntry)
