...
void LootItem::AddAlreadyLootedPlayer(const Player *player)
{
    alreadyLootedGUIDs.insert(player->GetGUIDLow());
}
...
bool Loot::FillLoot(uint32 lootId, LootStore const& store, Player* lootOwner, bool personal, bool noEmptyError, uint16 lootMode /*= LOOT_MODE_DEFAULT*/)
{
    ...
    } else FillNotNormalLootFor(lootOwner, true);
...
LootItem* Loot::LootItemInSlot(uint32 lootSlot, Player* player, QuestItem **qitem, QuestItem **ffaitem, QuestItem **conditem)
{
    ...
    else
    {
        ...
        AlreadyLootedPlayersSet const& alreadyLootedPlayers = item->GetAlreadyLootedPlayers();
        AlreadyLootedPlayersSet::const_iterator itr = alreadyLootedPlayers.find(player->GetGUIDLow());
        if(itr != alreadyLootedPlayers.end())
        {
            is_looted = true;
            return NULL;
        }
