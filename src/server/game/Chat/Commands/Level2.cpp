...
bool ChatHandler::HandleCharacterTitlesCommand(const char* args)
{
    ...
    Player* pTarget;
    if(!extractPlayerTarget((char*)args, &pTarget))
        return false;

    LocaleConstant loc = GetSessionDbcLocale();
    char const* targetName = pTarget->GetName();
    char const* knownStr = GetTrinityString(LANG_KNOWN);

    // Search in CharTitles.dbc
    for(uint32 id = 0; id < sCharTitlesStore.GetNumRows(); id++)
    {
        CharTitlesEntry const* pTitle = sCharTitlesStore.LookupEntry(id);
        if(pTitle && pTarget && pTarget->HasTitle(pTitle))
        {
            std::string name = pTitle->name[loc];
            if(name.empty())
                continue;

            char const* activeStr = pTarget && pTarget->GetUInt32Value(PLAYER_CHOSEN_TITLE) == pTitle->bit_index
                ? GetTrinityString(LANG_ACTIVE) : "";
            ...
            // send title in "id (idx:idx) - [namedlink locale]" format
            if(m_session)
                PSendSysMessage(LANG_TITLE_LIST_CHAT, id, pTitle->bit_index, id, titleNameStr, localeNames[loc], knownStr, activeStr);
            else
                PSendSysMessage(LANG_TITLE_LIST_CONSOLE, id, pTitle->bit_index, name.c_str(), localeNames[loc], knownStr, activeStr);
        }
    }
    return true;
}
...
bool ChatHandler::HandleMmapStatsCommand(const char* /*args*/)
{
    ...
    PSendSysMessage("  global pathfinding is %sabled", sWorld->getBoolConfig(CONFIG_PATHFINDING_ENABLED) ? "en" : "dis");
