...
bool ChatHandler::HandleGuildDeleteCommand(const char* args)
{
    ...
    while(Guild* pGuild = sGuildMgr->GetGuildByName(GuildName))
    {
        if(!pGuild)
            return false;
        pGuild->Disband();
...
{
    WorldObject* pWO = NULL;

    if(*args)
    {
        uint64 guid = extractGuidFromLink((char*)args);
        if(guid)
            pWO = (WorldObject*)ObjectAccessor::GetObjectByTypeMask(*m_session->GetPlayer(), guid, TYPEMASK_UNIT|TYPEMASK_GAMEOBJECT);
    ...
    PSendSysMessage(LANG_DISTANCE, m_session->GetPlayer()->GetDistance(pWO), m_session->GetPlayer()->GetDistance2d(pWO), m_session->GetPlayer()->GetExactDist(pWO), m_session->GetPlayer()->GetExactDist2d(pWO));
    return true;
}
...
{
    Player* pPlayer;
    if(!extractPlayerTarget((char*)args, &pPlayer))
        return false;

    pPlayer->SetHonorPoints(0);
    pPlayer->SetUInt32Value(PLAYER_FIELD_KILLS, 0);
    pPlayer->SetUInt32Value(PLAYER_FIELD_LIFETIME_HONORABLE_KILLS, 0);
    pPlayer->SetUInt32Value(PLAYER_FIELD_TODAY_CONTRIBUTION, 0);
    pPlayer->SetUInt32Value(PLAYER_FIELD_YESTERDAY_CONTRIBUTION, 0);
    pPlayer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_EARN_HONORABLE_KILL);
    return true;
}
...
bool ChatHandler::HandleBanAccountCommand(const char* args) { return HandleBanHelper(BAN_ACCOUNT, args); }

bool ChatHandler::HandleBanAccountByCharCommand(const char* args) { return HandleBanHelper(BAN_CHARACTER, args); }
...
bool ChatHandler::HandleBanIPCommand(const char* args) { return HandleBanHelper(BAN_IP, args); }
...
bool ChatHandler::HandleUnBanAccountCommand(const char* args) { return HandleUnBanHelper(BAN_ACCOUNT, args); }

bool ChatHandler::HandleUnBanAccountByCharCommand(const char* args) { return HandleUnBanHelper(BAN_CHARACTER, args); }
...
bool ChatHandler::HandleUnBanIPCommand(const char* args) { return HandleUnBanHelper(BAN_IP, args); }
