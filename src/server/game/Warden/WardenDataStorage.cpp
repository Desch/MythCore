...
void CWardenDataStorage::LoadWardenDataResult()
{
    ...
    QueryResult result = WorldDatabase.Query("SELECT `check`, `data`, `result`, `address`, `length`, `str`, `banDays`, `id` FROM warden_data_result WHERE `enabled` = 1");
    ...
    do
    {
        ...
        wd->banDays = fields[6].GetUInt8();
        wd->DBid = fields[7].GetUInt32();
