...
namespace VMAP
{
    ...
    {
        bool result = true;
        WmoLiquid* pWLiquid = new WmoLiquid();
        if(result && fread(&pWLiquid->iTilesX, sizeof(uint32), 1, rf) != 1)
            result = false;
        if(result && fread(&pWLiquid->iTilesY, sizeof(uint32), 1, rf) != 1)
            result = false;
        if(result && fread(&pWLiquid->iCorner, sizeof(Vector3), 1, rf) != 1)
            result = false;
        ...
        if(result && fread(pWLiquid->iHeight, sizeof(float), size, rf) != size)
            result = false;
        size = pWLiquid->iTilesX * pWLiquid->iTilesY;
        pWLiquid->iFlags = new uint8[size];
        if(result && fread(pWLiquid->iFlags, sizeof(uint8), size, rf) != size)
            result = false;
        if(!result)
            delete pWLiquid;
