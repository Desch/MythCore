...
namespace
{
    ...
    void ActionTeleport(Player * const player, Creature * const creature, const uint32 &id)
    {
        ...
        {
            std::string msg ("Vous n'avez pas le level requis. Cette destination requière le level " + ConvertStr(dest.m_level) + ".");
        ...
        {
            std::string msg ("Vous n'avez pas assez d'argents. Le prix de la téléportation est de " + ConvertMoney(dest.m_cost) + ".");
            creature->MonsterWhisper(msg.c_str(), player->GetGUID());
            return;
        }
