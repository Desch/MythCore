    ...
    struct boss_halion_realAI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
            ...
            if(pInstance->GetData(TYPE_ZARITHRIAN) == DONE)
            {
                ...
                {
                    DoScriptText(SAY_HALION_SPAWN, me);
                    intro = true;
                }
        ...
        void JustDied(Unit* /*pKiller*/)
        {
            ...
            DoScriptText(SAY_HALION_DEATH, me);
            ...
            for(Map::PlayerList::const_iterator i = pList.begin(); i != pList.end(); ++i)
            {
                if(Player* pPlayer = i->getSource())
                {
                    ...
                    if(!pPlayer->IsInMap(me))   continue;

                    pPlayer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, me->GetEntry());
                }
            }
        }
        ...
        {
            DoScriptText(RAND<int32>(SAY_HALION_SLAY_1,SAY_HALION_SLAY_2), me, pVictim);
        }
        ...
        void EnterCombat(Unit* /*pWho*/)
        {
            ...
            DoScriptText(SAY_HALION_AGGRO, me);
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            switch(getStage())
            {
                case 10: //PHASE INTRO
                    ...
                    {
                        me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_PASSIVE | UNIT_FLAG_DISABLE_MOVE);
                ...
                case 1: // Switch to phase 2
                    {
                        DoScriptText(SAY_HALION_PHASE_2, me);
                    ...
                    if(pInstance->GetData(TYPE_HALION_EVENT) == IN_PROGRESS)
                        {
                            ...
                            me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_FLAG_PASSIVE | UNIT_FLAG_NOT_SELECTABLE);
                ...
                case 6: // Switch to phase 3
                    DoScriptText(SAY_HALION_PHASE_3, me);
            ...
            if(m_uiEnrage <= diff)
            {
                ...
                m_uiEnrage = 600*IN_MILLISECONDS;
                DoScriptText(SAY_HALION_BERSERK, me);
...
public:
    ...
    struct mob_halion_controlAI : public Scripted_NoMovementAI
    {
        mob_halion_controlAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
        {
            pInstance = pCreature->GetInstanceScript();
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(m_uiCorporealityTimer <= diff)
            {
                ...
                float p_RealHP = pHalionReal->isAlive() ? Rdmg/p_MaxHP*100 : 0.0f;
...
public:
    ...
    struct mob_orb_rotation_focusAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            Creature* m_pulsar_N = me->GetMap()->GetCreature(pInstance->GetData64(NPC_SHADOW_PULSAR_N));
            if(!m_pulsar_N)
            {
                float x,y;
            ...
            Creature* m_pulsar_S = me->GetMap()->GetCreature(pInstance->GetData64(NPC_SHADOW_PULSAR_S));
            if(!m_pulsar_S)
            {
                float x,y;
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                Creature* m_pulsar_N = ObjectAccessor::GetCreature(*me, pInstance->GetData64(NPC_SHADOW_PULSAR_N));
                Creature* m_pulsar_S = ObjectAccessor::GetCreature(*me, pInstance->GetData64(NPC_SHADOW_PULSAR_S));
                if(!m_pulsar_N || !m_pulsar_S)
                    return; // timer will not be updated also. Should the pulsars be respawned?
...
public:
    ...
    struct mob_halion_orbAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_1 | UNIT_FLAG_OOC_NOT_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE);
        ...
        void StartMovement(uint32 id)
        {
            ...
            if(Creature* pFocus = me->GetMap()->GetCreature(pInstance->GetData64(NPC_ORB_ROTATION_FOCUS)))
                ...
                me->ForcedDespawn();
            me->GetMotionMaster()->Clear();
            me->GetMotionMaster()->MovePoint(id, x, y,  me->GetPositionZ()); // boss_halion.cpp(1182): warning : C6001: Using uninitialized memory 'x'.
        }
    ...
    struct mob_soul_consumptionAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            for(Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
            {
                if(Player* pPlayer = i->getSource())
                {
                    if(!grow && pPlayer->isAlive() && pPlayer->HasAura(SPELL_MARK_OF_CONSUMPTION))
                    {
                        Aura *pAura = pPlayer->GetAura(SPELL_MARK_OF_CONSUMPTION);
                        uint32 stacs = pAura->GetStackAmount();
                        m_Size += stacs*0.1f;
    ...
    struct mob_fiery_combustionAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            for(Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
            {
                if(Player* pPlayer = i->getSource())
                {
                    if(!grow && pPlayer->isAlive() && pPlayer->HasAura(SPELL_MARK_OF_COMBUSTION))
                    {
...
public:
    ...
    struct mob_halion_flameAI : public Scripted_NoMovementAI
    {
        mob_halion_flameAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
        {
            Reset();
        }
...
public:
    ...
    {
        pPlayer->CastSpell(pPlayer, SPELL_TWILIGHT_ENTER, false);
        return true;
    }
};
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, GameObject* /*pGo*/)
    {
        pPlayer->RemoveAurasDueToSpell(SPELL_TWILIGHT_ENTER);
        return true;
    }
};
