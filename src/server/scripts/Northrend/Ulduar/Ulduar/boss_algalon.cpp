...
public:
    ...
    struct mob_black_holeAI : public ScriptedAI
    {
        ...
        {
            if(pInstance && pInstance->GetBossState(BOSS_ALGALON) != IN_PROGRESS)
                me->ForcedDespawn();
...
public:
    ...
    struct boss_algalonAI : public ScriptedAI
    {
        ...
        {
            if(pInstance)
                pInstance->SetBossState(BOSS_ALGALON, FAIL);
        ...
        void DoOutro()
        {
            if(pInstance)
            {
                ...
                pInstance->DoUpdateWorldState(UI_STATE_ALGALON_TIMER_ON, 0);
                // hacky way to complete achievements; use only if you have this function
                pInstance->DoCompleteAchievement(m_bIsRegularMode ? ACHIEV_OBSERVED : ACHIEV_OBSERVED_H);

                if(m_bFeedOnTears)
                    pInstance->DoCompleteAchievement(m_bIsRegularMode ? ACHIEV_FEED_TEARS : ACHIEV_FEED_TEARS_H);
            }
        ...
        void Aggro(Unit* pWho)
        {
            ...
            if(m_bIsFirstTime)
            {
                ...
                if(pInstance)
                {
                    pInstance->DoUpdateWorldState(UI_STATE_ALGALON_TIMER_ON, 1);
                    pInstance->DoUpdateWorldState(UI_STATE_ALGALON_TIMER_COUNT, m_uiDespawnTimer / 60000);
                }
            } else DoScriptText(SAY_AGGRO, me);
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if(m_uiDespawnTimer < m_uiLastTimer - 60000 && !m_bIsDespawned)
            {
                ...
                if(pInstance)
                    pInstance->DoUpdateWorldState(UI_STATE_ALGALON_TIMER_COUNT, tMinutes);
            }
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, GameObject* pGo)
    {
        ...
        if(!m_bHasItem || !pInstance)
            return false;

        // disable if encounter is already done
        if(pInstance->GetBossState(BOSS_ALGALON) == DONE)
        {
        ...
        // start encounter
        if(Creature* pAlgalon = pGo->GetMap()->GetCreature(pInstance->GetData64(DATA_ALGALON)))
        {
            if(pAlgalon->isAlive())
            {
                ...
                // open celestial door
                if(GameObject* pDoor = pInstance->instance->GetGameObject(pInstance->GetData64(GO_CELESTIAL_DOOR)))
                    pInstance->DoUseDoorOrButton(pDoor->GetGUID());
            }
        }
...
public:
    ...
    class spell_algalon_phase_punch_AuraScript : public AuraScript
    {
        ...
        void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            ...
            {
                pTarget->RemoveAurasDueToSpell(SPELL_PHASE_PUNCH_ALPHA_1);
                pTarget->RemoveAurasDueToSpell(SPELL_PHASE_PUNCH_ALPHA_2);
                pTarget->RemoveAurasDueToSpell(SPELL_PHASE_PUNCH_ALPHA_3);
                pTarget->RemoveAurasDueToSpell(SPELL_PHASE_PUNCH_ALPHA_4);
            }
        }
...
{
public:
    go_celestial_console() : GameObjectScript("go_celestial_console") { }
