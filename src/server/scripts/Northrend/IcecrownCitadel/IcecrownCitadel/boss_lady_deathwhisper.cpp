...
class boss_lady_deathwhisper : public CreatureScript
{
    public:
        ...
        struct boss_lady_deathwhisperAI : public BossAI
        {
            ...
            {
                if(!_introDone && me->IsWithinDistInMap(pWho, 110.0f))
                {
                    _introDone = true;
                    Talk(SAY_INTRO_1);
            ...
            void AttackStart(Unit* pVictim)
            {
                ...
                if(pVictim && me->Attack(pVictim, true) && !(events.GetPhaseMask() & PHASE_ONE_MASK))
                    me->GetMotionMaster()->MoveChase(pVictim);
            }

            void EnterCombat(Unit* pWho)
            {
                if(!instance->CheckRequiredBosses(DATA_LADY_DEATHWHISPER, pWho->ToPlayer()))
                {
                    EnterEvadeMode();
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_P2_FROSTBOLT:
                            DoCastVictim(RAID_MODE<uint32>(SPELL_FROSTBOLT_10N, SPELL_FROSTBOLT_25N, SPELL_FROSTBOLT_10H, SPELL_FROSTBOLT_25H));
        ...
        {
            return GetIcecrownCitadelAI<boss_lady_deathwhisperAI>(pCreature);
        }
};
...
class npc_cult_fanatic : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return GetIcecrownCitadelAI<npc_cult_fanaticAI>(pCreature);
        }
};
...
class npc_vengeful_shade : public CreatureScript
{
    public:
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return GetIcecrownCitadelAI<npc_vengeful_shadeAI>(pCreature);
        }
};
...
class npc_darnavan : public CreatureScript
{
    public:
        ...
        {
            npc_darnavanAI(Creature* pCreature): ScriptedAI(pCreature) { }
        ...
        CreatureAI* GetAI(Creature* pCreature) const
        {
            return GetIcecrownCitadelAI<npc_darnavanAI>(pCreature);
        }
};
