...
public:
    ...
    struct boss_deathbringer_saurfangAI : public BossAI
    {
        ...
        {
            if(me->isDead()) // Possible, shit happen here, pointed by NeorQ
                return;
                ...
                for(Map::PlayerList::const_iterator itr = Players.begin(); itr != Players.end(); ++itr)
                {
                    if(Player* player = itr->getSource())
                    {
                        ...
                        if(player->HasAura(SPELL_MARK_OF_THE_FALLEN_CHAMPION))
                            DoCast(player, RAID_MODE<uint32>(72255, 72444, 72445, 72446));
                    }
                }
            }
        }
    ...
    {
        return GetIcecrownCitadelAI<boss_deathbringer_saurfangAI>(pCreature);
    }
};
...
public:
    ...
    {
        npc_high_overlord_saurfangAI(Creature* pCreature): ScriptedAI(pCreature)
        {
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return GetIcecrownCitadelAI<npc_high_overlord_saurfangAI>(pCreature);
    }
};
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return GetIcecrownCitadelAI<npc_muradin_bronzebeard_iccAI>(pCreature);
    }
};
...
public:
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return GetIcecrownCitadelAI<npc_saurfang_eventAI>(pCreature);
    }
};
...
public:
    ...
    {
        npc_blood_beastAI(Creature* pCreature): ScriptedAI(pCreature)
        {
            _instance = me->GetInstanceScript();
        }
         void DamageDealt(Unit* /*pVictim*/, uint32& /*damage*/, DamageEffectType /*damageType*/)
    ...
    CreatureAI* GetAI(Creature* pCreature) const
    {
        return GetIcecrownCitadelAI<npc_blood_beastAI>(pCreature);
    }
};
