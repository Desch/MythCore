...
bool IsVampire(Unit const* pUnit)
{
    for(uint8 i = 0; i < 3; ++i)
        if(pUnit && pUnit->GetMap()->GetSpawnMode() < 4)
            if(pUnit->HasAura(vampireAuras[i][pUnit->GetMap()->GetSpawnMode()]))
                return true;
    return false;
}
...
class boss_blood_queen_lana_thel : public CreatureScript
{
    public:
        ...
        {
            return GetIcecrownCitadelAI<boss_blood_queen_lana_thelAI>(pCreature);
        }
};
...
class spell_blood_queen_vampiric_bite : public SpellScriptLoader
{
    public:
        ...
        class spell_blood_queen_vampiric_bite_SpellScript : public SpellScript
        {
            ...
            {
                if(IsVampire(GetTargetUnit()) || GetTargetUnit()->GetTypeId() != TYPEID_PLAYER || GetTargetUnit()->GetAreaId() != 4891)
                {
...
class achievement_once_bitten_twice_shy_n : public AchievementCriteriaScript
{
    public:
        ...
        bool OnCheck(Player* pSource, Unit* pTarget)
        {
            ...
            if(LanaThelAI* lanaThelAI = CAST_AI(LanaThelAI, pTarget->GetAI()))
                return !lanaThelAI->WasVampire(pSource->GetGUID());
            return false;
        }
};
