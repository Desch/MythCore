        ...
        void OnCreatureCreate(Creature* pCreature)
        {
            ...
            switch(pCreature->GetEntry())
            {
                ...
                case NPC_FREED_HORDE_SLAVE1:
                    if(_teamInInstance == ALLIANCE)
                        pCreature->UpdateEntry(NPC_FREED_ALLIANCE_SLAVE1, ALLIANCE);
                    break;
                case NPC_FREED_HORDE_SLAVE2:
                    if(_teamInInstance == ALLIANCE)
                        pCreature->UpdateEntry(NPC_FREED_ALLIANCE_SLAVE2, ALLIANCE);
                    break;
                case NPC_FREED_HORDE_SLAVE3:
                    if(_teamInInstance == ALLIANCE)
                        pCreature->UpdateEntry(NPC_FREED_ALLIANCE_SLAVE3, ALLIANCE);
                    break;
                ...
                case NPC_GORKUN_IRONSKULL_FREED:
                    if(_teamInInstance == ALLIANCE)
                        pCreature->UpdateEntry(NPC_MARTIN_VICTUS_FREED, ALLIANCE);
                    _victusOrGorkunFreedGUID = pCreature->GetGUID();
                    break;
                case NPC_GORKUN_IRONSKULL:
                    if(_teamInInstance == ALLIANCE)
                        pCreature->UpdateEntry(NPC_MARTIN_VICTUS, ALLIANCE);
                    break;
                case NPC_SINDRAGOSA:
                    _sindragosaGUID = pCreature->GetGUID();
                    break;
                default:
                    break;
            }
