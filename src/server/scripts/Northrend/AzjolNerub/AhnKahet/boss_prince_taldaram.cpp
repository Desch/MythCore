...
public:
    ...
    {
        mob_taldaram_flamesphereAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            pInstance = pCreature->GetInstanceScript();
        }
...
public:
    ...
    bool OnGossipHello(Player* /*pPlayer*/, GameObject* pGO)
    {
        ...
        {
            if(Creature* pPrinceTaldaram = Unit::GetCreature(*pGO, pInstance->GetData64(DATA_PRINCE_TALDARAM)))
            {
                if(pPrinceTaldaram->isAlive())
                {
