...
public:
    ...
    {
        mob_palehoof_orbAI(Creature* pCreature) : ScriptedAI(pCreature)
        {
            pInstance = pCreature->GetInstanceScript();
        }
        ...
        void MovementInform(uint32 /*type*/, uint32 id)
        {
            ...
            switch(id)
            {
                case PHASE_FRENZIED_WORGEN:     pNext = Unit::GetCreature((*me), pInstance->GetData64(DATA_MOB_FRENZIED_WORGEN));   break;
                case PHASE_RAVENOUS_FURLBORG:   pNext = Unit::GetCreature((*me), pInstance->GetData64(DATA_MOB_RAVENOUS_FURBOLG));  break;
                case PHASE_MASSIVE_JORMUNGAR:   pNext = Unit::GetCreature((*me), pInstance->GetData64(DATA_MOB_MASSIVE_JORMUNGAR)); break;
                case PHASE_FEROCIOUS_RHINO:     pNext = Unit::GetCreature((*me), pInstance->GetData64(DATA_MOB_FEROCIOUS_RHINO));   break;
                case PHASE_GORTOK_PALEHOOF:     pNext = Unit::GetCreature((*me), pInstance->GetData64(DATA_GORTOK_PALEHOOF));       break;
            }
...
public:
    ...
    bool OnGossipHello(Player* /*pPlayer*/, GameObject* pGO)
    {
        ...
        {
            Creature* pPalehoof = Unit::GetCreature(*pGO, pInstance->GetData64(DATA_GORTOK_PALEHOOF));
            if(pPalehoof && pPalehoof->isAlive())
            {
