...
public:
    ...
    class spell_temperature_reading_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            {
                if(GameObject* pGo = pPlayer->FindNearestGameObject(GO_HOT_POINT, 10.0f))
                {
                    if(pGo->GetPositionZ() == -188 && pPlayer->GetQuestStatus(974) == QUEST_STATUS_INCOMPLETE)
                        pPlayer->KilledMonsterCredit(10541, 0);
                }
            }
        }
...
enum eQuest12029Data
{
    NPC_FAMISHED_SCOURGE_TROLL          = 26570,
    SPELL_BURNINATE_KILL_CREDIT         = 47208
};

class spell_q12029_burninate : public SpellScriptLoader
{
public:
    spell_q12029_burninate() : SpellScriptLoader("spell_q12029_burninate") { }

    class spell_q12029_burninate_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_q12029_burninate_AuraScript)
        void ApplyEffect(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if(Unit* pCaster = GetCaster())
            {
                ...
                {
                    if(pTarget->GetEntry() == NPC_FAMISHED_SCOURGE_TROLL)
                    {
                        pCaster->CastSpell(pCaster, SPELL_BURNINATE_KILL_CREDIT, true);
        ...
        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_q12029_burninate_AuraScript::ApplyEffect, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_q12029_burninate_AuraScript();
    }
};
