...
public:
    ...
    class spell_gen_gunship_portal_SpellScript : public SpellScript
    {
        ...
        void HandleScript(SpellEffIndex /*effIndex*/)
        {
            ...
            if(Battleground* pBG = caster->ToPlayer()->GetBattleground())
                if(pBG->GetTypeID(true) == BATTLEGROUND_IC)
                    pBG->DoAction(1, caster->GetGUID());
        }
...
public:
    ...
    class spell_gen_luck_of_the_draw_AuraScript : public AuraScript
    {
        ...
        void CalcPeriodic(AuraEffect const* /*pEffect*/, bool& isPeriodic, int32& amplitude)
        {
            isPeriodic = true;
            amplitude = 5 * IN_MILLISECONDS;
        }

        void Update(AuraEffect* /*pEffect*/)
        {
...
public:
    ...
    class spell_gen_break_shield_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            switch(effIndex)
            {
                ...
                case EFFECT_1: // On damaging spells, for removing a defend layer
                {
                    ...
                    {
                        if(Aura* pAura = itr->second->GetBase())
                        {
                            if(SpellEntry const* pSpellEntry = pAura->GetSpellProto())
                            {
                                if(pAura && pSpellEntry->SpellIconID == 2007 && pAura->HasEffectType(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN))
                                {
                                    pAura->ModStackAmount(-1, AURA_REMOVE_BY_ENEMY_SPELL);
                                    // Remove dummys from rider (Necessary for updating visual shields)
                                    if(Unit* rider = target->GetCharmer())
                                        if(Aura* defend = rider->GetAura(pAura->GetId()))
        ...
        void Register()
        {
            OnEffect += SpellEffectFn(spell_gen_break_shield_SpellScript::HandleScriptEffect, EFFECT_FIRST_FOUND, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    class spell_gen_mounted_charge_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex effIndex)
        {
            ...
            switch(effIndex)
            {
                case EFFECT_0: // On spells wich trigger the damaging spell (and also the visual)
                {
                    ...
                    if(Unit* pVehicle = GetCaster()->GetVehicleBase())
                        pVehicle->CastSpell(target, spellId, false);
                    else
                        GetCaster()->CastSpell(target, spellId, false);
                    break;
                }
                ...
                case EFFECT_2:
                {
                    ...
                    for(Unit::AuraApplicationMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
                    {
                        if(Aura* pAura = itr->second->GetBase())
                        {
                            if(SpellEntry const* pSpellEntry = pAura->GetSpellProto())
                            {
                                if(pSpellEntry->SpellIconID == 2007 && pAura->HasEffectType(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN))
                                {
                                    pAura->ModStackAmount(-1, AURA_REMOVE_BY_ENEMY_SPELL);
                                    // Remove dummys from rider (Necessary for updating visual shields)
                                    if(Unit* pCharmer = target->GetCharmer())
                                        if(Aura* pAura2 = pCharmer->GetAura(pAura->GetId()))
                                            pAura2->ModStackAmount(-1, AURA_REMOVE_BY_ENEMY_SPELL);
                                    break;
                                }
                            }
                        }
        ...
        void HandleChargeEffect(SpellEffIndex effIndex)
        {
            ...
            if(Unit* pRider = GetCaster()->GetCharmer())
                pRider->CastSpell(GetHitUnit(), spellId, false);
            else
        ...
        void Register()
        {
            SpellEntry const* spell = sSpellStore.LookupEntry(m_scriptSpellId);

            if(spell->Effect[EFFECT_0] == (SPELL_EFFECT_SCRIPT_EFFECT))
                OnEffect += SpellEffectFn(spell_gen_mounted_charge_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            else if(spell->Effect[EFFECT_1] == (SPELL_EFFECT_SCRIPT_EFFECT))
                OnEffect += SpellEffectFn(spell_gen_mounted_charge_SpellScript::HandleScriptEffect, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
            else if(spell->Effect[EFFECT_2] == (SPELL_EFFECT_SCRIPT_EFFECT))
                OnEffect += SpellEffectFn(spell_gen_mounted_charge_SpellScript::HandleScriptEffect, EFFECT_2, SPELL_EFFECT_SCRIPT_EFFECT);

            if(spell->Effect[EFFECT_0] == SPELL_EFFECT_CHARGE)
                OnEffect += SpellEffectFn(spell_gen_mounted_charge_SpellScript::HandleChargeEffect, EFFECT_0, SPELL_EFFECT_CHARGE);
            else if(spell->Effect[EFFECT_1] == SPELL_EFFECT_CHARGE)
                OnEffect += SpellEffectFn(spell_gen_mounted_charge_SpellScript::HandleChargeEffect, EFFECT_1, SPELL_EFFECT_CHARGE);
            else if(spell->Effect[EFFECT_2] == SPELL_EFFECT_CHARGE)
                OnEffect += SpellEffectFn(spell_gen_mounted_charge_SpellScript::HandleChargeEffect, EFFECT_2, SPELL_EFFECT_CHARGE);
        }
    };
...
public:
    ...
    class spell_gen_defendAuraScript : public AuraScript
    {
        ...
        bool Validate(SpellEntry const* /*pSpellEntry*/)
        {
            if(!sSpellStore.LookupEntry(SPELL_VISUAL_SHIELD_1))
                return false;
            if(!sSpellStore.LookupEntry(SPELL_VISUAL_SHIELD_2))
                return false;
            if(!sSpellStore.LookupEntry(SPELL_VISUAL_SHIELD_3))
                return false;
            return true;
        }
        ...
        void RefreshVisualShields(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            if(Unit* caster = GetCaster())
            {
                ...
                for(uint8 i = 0; i < GetSpellProto()->StackAmount; ++i)
        ...
        void Register()
        {
            ...
            if(spell->EffectApplyAuraName[EFFECT_0] == SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN)
            {
            ...
            if(spell->EffectApplyAuraName[EFFECT_2] == SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN)
            ...
            if(spell->EffectApplyAuraName[EFFECT_1] == SPELL_AURA_DUMMY)
            {
...
public:
    ...
    class spell_gen_tournament_duel_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellEntry const* /*pSpellEntry*/)
        {
            if(!sSpellStore.LookupEntry(SPELL_ON_TOURNAMENT_MOUNT))
                return false;
            if(!sSpellStore.LookupEntry(SPELL_MOUNTED_DUEL))
                return false;
            return true;
        }
        ...
        void Register()
        {
            OnEffect += SpellEffectFn(spell_gen_tournament_duel_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
        }
    };
...
public:
    ...
    class spell_gen_summon_tournament_mount_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellEntry const* /*pSpellEntry*/)
        {
            if(!sSpellStore.LookupEntry(SPELL_LANCE_EQUIPPED))
                return false;
            return true;
        }
    ...
    class spell_gen_on_tournament_mountAuraScript : public AuraScript
    {
        ...
        void HandleRemoveEffect(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if(Unit* pCaster = GetCaster())
                pCaster->RemoveAurasDueToSpell(_pennantSpellId);
        }
        ...
        uint32 GetPennatSpellId(Player* player, Unit* mount)
        {
            switch(mount->GetEntry())
            {
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_STORMWIND)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_GNOMEREGAN)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_SEN_JIN)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_SILVERMOON)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_DARNASSUS)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_THE_EXODAR)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_IRONFORGE)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_UNDERCITY)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_ORGRIMMAR)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_THUNDER_BLUFF)))
                ...
                {
                    if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_ALLIANCE)) || player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_CHAMPION_HORDE)))
                        return player->getClass() == CLASS_DEATH_KNIGHT ? SPELL_PENNANT_EBON_BLADE_CHAMPION : SPELL_PENNANT_ARGENT_CRUSADE_CHAMPION;
                    else if(player->GetAchievementMgr().HasAchieved(sAchievementStore.LookupEntry(ACHIEVEMENT_ARGENT_VALOR)))
...
public:
    ...
    class spell_gen_ribbon_pole_dancer_check_AuraScript : public AuraScript
    {
        ...
        void PeriodicTick(AuraEffect const* /*aurEff*/)
        {
            ...
            } else target->AddAura(SPELL_RIBBON_DANCE, target);
        }
