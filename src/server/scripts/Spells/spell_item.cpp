...
public:
    ...
    class spell_item_rocket_boots_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellEntry const* /*pSpellEntry*/)
        {
            if(!sSpellStore.LookupEntry(30452))
                return false;
            return true;
        }
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if(GetCaster()->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if(Battleground* pGB = pCaster->GetBattleground())
                    pGB->EventPlayerDroppedFlag(pCaster);

                pCaster->RemoveSpellCooldown(30452);
                pCaster->CastSpell(pCaster, 30452, true, NULL);
            }
        }

        void Register()
        {
            OnEffect += SpellEffectFn(spell_item_rocket_boots_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
